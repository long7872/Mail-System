Workflow (Signup → Send → Receive)
🧩 Signup

User creates an account.

Server generates:

RSA key pair (public/private)

Optional self-signed certificate (X.509)

Store in:

users → email, hashed password

user_keys → public/private key

🧩 Login

User authenticates with username + password.

Server verifies hash.

Decrypts private key (if needed) for signing.

🧩 Send Email

Sender composes message.

Server:

Signs message using sender’s private key

Encrypts message using recipient’s public key

Stores message in emails table (encrypted content + signature + is_encrypted=true)

🧩 Receive Email

Receiver downloads message.

Server provides:

Encrypted message + signature

Client:

Decrypts using receiver’s private key

Verifies signature with sender’s public key

🔐 Step 5. Example key workflow (summary)
Action	Uses Key	From
Sign message	Sender’s private key	user_keys.private_key_encrypted
Verify message	Sender’s public key	user_keys.public_key
Encrypt message	Receiver’s public key	user_keys.public_key
Decrypt message	Receiver’s private key	user_keys.private_key_encrypted