...
router.get('/received', auth, async (req, res) => {
	try {
		...
		if (email != "SPAM" || email != "HAM") {
			return res.status(400).json({ message: 'ERROR ENUM TYPE' });
		}

		const [rows] = await pool.query(
            `SELECT e.id, u.email AS sender_email, e.encrypted_key_recipient, e.ciphertext, e.iv, e.ml_scan_status,
				e.ml_spam_result, e.ml_spam_score, e.ml_sentiment_result, e.ml_sentiment_score, e.created_at
             FROM emails e
