...
router.get('/received', auth, async (req, res) => {
	try {
		...
		console.log("======= /received API CALLED =======");
		console.log("User ID:", userId);
		console.log("Query type:", type);
		console.log("====================================");

		// Validate ENUM
		const VALID_TYPES = ["SPAM", "HAM", "ALL"];

		if (!VALID_TYPES.includes(type)) {
			console.warn("Invalid type provided:", type);
			return res.status(400).json({ message: 'ERROR ENUM TYPE. Allowed: SPAM | HAM | ALL' });
		}
		...
		if (type === "ALL") {
			sql = `
				SELECT 
					e.id, u.email AS sender_email,
					e.encrypted_key_recipient, e.ciphertext, e.iv,
					e.ml_scan_status, e.ml_spam_result, e.ml_spam_score,
					e.ml_sentiment_result, e.ml_sentiment_score,
					e.created_at
				FROM emails e
			...
			console.log("Query mode: ALL emails (no spam filter)");
		} else {
			sql = `
				SELECT 
					e.id, u.email AS sender_email,
				...
				FROM emails e
				JOIN users u ON e.sender_id = u.id
				WHERE e.recipient_id = ? AND e.ml_spam_result = ?
				ORDER BY e.created_at DESC
			`;
			params = [userId, type];

			console.log(`Query mode: FILTER BY TYPE = ${type}`);
		}
