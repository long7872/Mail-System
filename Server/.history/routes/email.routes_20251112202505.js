...
router.post('/send', auth, async (req, res) => {
	...
	try {
		...
		try {
			...
			const { spam_score } = response.data;
			console.log("ML Spam score:", spam_score);

			// cập nhật DB
			await pool.query(
				`UPDATE emails
				SET ml_scan_status = ?, ml_spam_result = ?, ml_spam_score = ?, ml_sentiment_label = ?, ml_sentiment_score = ?
				WHERE id = ?`,
				[EmailStatus.SCANNED, spam_score, result.insertId]
			);
