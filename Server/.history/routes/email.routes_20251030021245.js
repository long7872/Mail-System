...
router.post('/send', auth, async (req, res) => {
  ...
  try {
    ...
    // 🔏 Sign message
    const privateKeyPem = forge.pki.privateKeyFromPem(forge.pki.privateKeyToPem(
      forge.pki.privateKeyFromPem(
        forge.pki.privateKeyToPem(forge.pki.privateKeyFromPem(''))
      )
    )); // placeholder for later decrypt with password

    const md = forge.md.sha256.create();
    md.update(content, 'utf8');
    const signature = forge.util.encode64(privateKeyPem.sign(md));
