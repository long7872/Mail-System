...
router.get('/received', auth, async (req, res) => {
	try {
		...
		// Validate ENUM
		const VALID_TYPES = ["SPAM", "HAM"];

		if (!VALID_TYPES.includes(type)) {
			console.warn("❌ Invalid type provided:", type);
			return res.status(400).json({ message: 'ERROR ENUM TYPE. Allowed: SPAM | HAM' });
		}

		console.log(`✅ FILTER: Only show emails with ml_spam_result = '${type}'`);
		...
		const sql = `
			SELECT 
				...
				e.encrypted_key_recipient, 
				e.ciphertext, 
				e.iv,
				e.ml_scan_status,
				e.ml_spam_result,
				e.ml_spam_score,
				e.ml_sentiment_result,
				e.ml_sentiment_score,
				e.created_at
			FROM emails e
		...
		const [rows] = await pool.query(sql, [userId, type]);
